...
select customer_id , count(order_id) as "Total count"
from order_detail
group by 1
order by 2 desc
limit 3;
...
SELECT restaurant_id, AVG(rrating) AS avg_rating
FROM restaurant
...
select f.food_id, f.food_name,  sum(f.price_per_unit*ofo.quantity) as "Total Revenue"
from foods f
inner join  order_food ofo
on f.food_id = ofo.food_id
group by 1,2
...
SELECT 
    r.restaurant_id,
    r.restaurant_name,
    SUM(ofo.quantity * f.price_per_unit) AS total_revenue
FROM restaurant r
inner JOIN  order_food ofo
on r.restaurant_id = ofo.restaurant_id 
...
-- 6. Find the 5 most popular food items based on the quantity sold.
 ...
 SELECT f.food_id, f.food_name , SUM(ofo.quantity) AS total_quantity_sold
FROM  order_food ofo
JOIN foods f ON ofo.food_id = f.food_id
...
select month(order_time) as "Month", count(order_id) as "Total Orders"
from order_detail
group by 1
...
select c.customer_id, c.customer_name,
    round(avg(o_f.quantity * f.price_per_unit), 2) as avg_order_amount
from customer c
join order_detail od on c.customer_id = od.customer_id
join order_food o_f on od.order_id = o_f.order_id
join foods f on o_f.food_id = f.food_id
...
join 
    restaurant r on od.restaurant_id = r.restaurant_id
group by 
    r.restaurant_id, r.restaurant_name, c.customer_id, c.customer_name
having 
    count(od.order_id) = (
        select 
            max(order_count)
        from (
            select 
                count(od2.order_id) as order_count
            from 
                order_detail od2
            where 
                od2.restaurant_id = r.restaurant_id
            group by 
                od2.customer_id
        ) as subquery
    )
...
select count(order_id) as weekend_order_count
from order_detail
...
select r.restaurant_name, count(o_f.food_id) as menu_size
from restaurant r
join order_food  o_f on r.restaurant_id = o_f.restaurant_id
...
select payment_type , sum(order_food.quantity*foods.price_per_unit) as "Total Payment Amount"
from payment_table
inner join order_food on order_food.order_id = payment_table.order_id
inner join foods on foods.food_id = order_food.food_id
group by 1;
